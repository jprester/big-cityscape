...
import { generateSyntheticCity, DEFAULT_SYNTHETIC_CITY_CONFIG } from '../src/city/synthetic/generation/generateSyntheticCity';
...
import type { SyntheticBlockDefinition, SyntheticBounds2 } from '../src/city/synthetic/model/proofDistrict';
...
const seed = Number(argumentsByName.get('seed') ?? DEFAULT_SYNTHETIC_CITY_CONFIG.seed);
...
const resolvedBuildings = (Object.keys(manifests) as TexturedBuildingPackId[])
  .flatMap((packId) => {
    ...
    return plansByPack[packId].assignments.map(({ modelId, placement }) => {
      ...
      if (model === undefined) {
        throw new Error(`Look-development layout lost ${packId} model ${modelId}.`);
      }
      return {
        id: placement.id,
...
const allUnfilledPlacementIds = (Object.keys(manifests) as TexturedBuildingPackId[])
...
function readManifest(packId: TexturedBuildingPackId): PackManifest {
  ...
  if (
    ...
    throw new Error(`Unsupported ${packId} look-development manifest: ${filePath}.`);
  }
...
function readNamedArguments(argumentsList: readonly string[]): Map<string, string> {
  ...
  for (let index = 0; index < argumentsList.length; index += 2) {
    ...
    if (name === undefined || !name.startsWith('--') || value === undefined) {
      throw new Error('Arguments must be --name value pairs: --seed or --output.');
    }
